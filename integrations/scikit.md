...
한 클러스터의 한 포인트가 이웃 클러스터의 포인트에 얼마나 가까운지 측정하고 나타냅니다. 클러스터의 두께는 클러스터 사이즈에 상응합니다. 수직선은 모든 포인트의 평균 실루엣 값을 나타냅니다.
