...
These values can be set in four ways: as command-line arguments to [`wandb sweep`](../../ref/cli/wandb-sweep.md), as part of the [sweep configuration](broken-reference) YAML file, as [environment variables](../track/advanced/environment-variables.md), or via the `wandb/settings` file.
...
To pass hyperparameters as boolean flags you can use the `${args_no_boolean_flags}` macro in the [command section of the config](broken-reference). This will automatically pass in any boolean parameters as flags: when `param` is `True` the command will receive `--param`, when `param` is `False` the flag will be omitted.&#x20;
